...
class CarState(object):
    ...
    def setFromMsg(self, str_sensors):
        ...
        sensory = self.sensors

        
        keys = self.sensors.keys()

        sensory = dict(self.sensors)
        for key in keys:
            if len(self.sensors[key]) > 1:
                arr = np.array(sensory[key])
                sensory[key] = [arr]


        tempdata=pd.DataFrame(sensory)
        self.dataset = pd.concat([self.dataset, tempdata], axis=0)
